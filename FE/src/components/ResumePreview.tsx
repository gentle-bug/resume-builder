  ...
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex' }}>
      ...
      <div style={{ flex: 1, padding: '36px 28px' }}>
        <h1 style={{ fontSize: 28, fontWeight: 700, color: '#064E3B', margin: 0 }}>{basics.name || 'Your Name'}</h1>
        {basics.label && <p style={{ color: primary, fontSize: 14, margin: '4px 0 0' }}>{basics.label}</p>}
        {basics.summary && (
          <div style={{ marginTop: 24 }}>
            <h3 style={{ ...headingStyle, color: '#064E3B' }}>Summary</h3>
            <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, margin: 0 }}>{basics.summary}</p>
          </div>
        )}
        {has(work) && (
          <div style={{ marginTop: 24 }}>
            <h3 style={{ ...headingStyle, color: '#064E3B' }}>Experience</h3>
            {work.map((w, i) => (
              <div key={i} style={{ marginBottom: 16 }}>
        ...
        {has(projects) && (
          <div style={{ marginTop: 24 }}>
            <h3 style={{ ...headingStyle, color: '#064E3B' }}>Projects</h3>
            {projects.map((p, i) => (
              <div key={i} style={{ marginBottom: 14 }}>
...
const Malbec: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex' }}>
      {/* Sidebar */}
      <div style={{ width: 260, background: `${primary}20` }}>
        ...
        <div style={{ padding: '20px 22px' }}>
          <div style={{ fontSize: 12, color: '#4C1D95', lineHeight: 1.8, marginBottom: 20 }}>
            {basics.email && <div>{basics.email}</div>}
            {basics.phone && <div>{basics.phone}</div>}
            {basics.location && <div>{basics.location}</div>}
          </div>
          {has(skills) && (
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 1.5, color: '#4C1D95', margin: '0 0 10px', fontWeight: 700, borderBottom: `2px solid ${primary}60`, paddingBottom: 4 }}>Skills</h3>
              {skills.map((s, i) => (
                <div key={i} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#1E1B4B' }}>{s.name}{s.level && <span style={{ fontWeight: 400, color: '#7C3AED' }}> ({s.level})</span>}</div>
                  {has(s.keywords) && <div style={{ fontSize: 10, color: '#6D28D9', marginTop: 2 }}>{s.keywords.join(', ')}</div>}
                </div>
              ))}
            </div>
          )}
          {has(education) && (
            <div>
              <h3 style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 1.5, color: '#4C1D95', margin: '0 0 10px', fontWeight: 700, borderBottom: `2px solid ${primary}60`, paddingBottom: 4 }}>Education</h3>
              {education.map((e, i) => (
                <div key={i} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#1E1B4B' }}>{e.studyType ? `${e.studyType} in ` : ''}{e.area}</div>
                  <div style={{ fontSize: 11, color: '#6D28D9' }}>{e.institution}</div>
                  <div style={{ fontSize: 10, color: '#A78BFA' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
                </div>
              ))}
            </div>
          )}
...
const Champagne: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex' }}>
      ...
      <div style={{ flex: 1, padding: '30px 24px' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {work.map((w, i) => (
              <div key={i} style={{ marginBottom: 14, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 12, color: primary }}>{w.company} | {w.startDate}{w.endDate ? ` - ${w.endDate}` : ''}</div>
                {w.summary && <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0', lineHeight: 1.6 }}>{w.summary}</p>}
                {has(w.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#374151', lineHeight: 1.6, textAlign: 'left', display: 'inline-block' }}>{w.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
          </div>
        )}
        {has(projects) && (
          <div>
            ...
            {projects.map((p, i) => (
              <div key={i} style={{ marginBottom: 12, textAlign: 'center' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{p.name}</div>
                {p.url && <div style={{ fontSize: 11, color: primary }}>{p.url}</div>}
                {p.description && <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0', lineHeight: 1.6 }}>{p.description}</p>}
                {has(p.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#374151', lineHeight: 1.6, textAlign: 'left', display: 'inline-block' }}>{p.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
          </div>
        )}
      ...
      {/* Sidebar right */}
      <div style={{ width: 220, background: '#FEF2F2', padding: '30px 18px', borderLeft: `3px solid ${primary}` }}>
        {has(skills) && (
          <div style={{ marginBottom: 24 }}>
            ...
            {skills.map((s, i) => (
              <div key={i} style={{ marginBottom: 8, textAlign: 'center' }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#991B1B' }}>{s.name}</div>
                {s.level && <div style={{ fontSize: 10, color: '#DC2626' }}>{s.level}</div>}
                {has(s.keywords) && <div style={{ fontSize: 10, color: '#B91C1C', marginTop: 2 }}>{s.keywords.join(', ')}</div>}
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom: 10, textAlign: 'center' }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#991B1B' }}>{e.studyType ? `${e.studyType} in ` : ''}{e.area}</div>
                <div style={{ fontSize: 11, color: '#B91C1C' }}>{e.institution}</div>
                <div style={{ fontSize: 10, color: '#EF4444' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
              </div>
            ))}
          </div>
        )}
...
const PinotGrigio: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  const { basics, work, education, skills, projects } = data
  const primary = color
  const heading: React.CSSProperties = { fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: '#166534', fontWeight: 700, margin: '0 0 12px' }
  return (
    <div style={{ ...A4, background: '#fff' }}>
      ...
      <div style={{ background: `${primary}18`, padding: '28px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#14532D', margin: 0 }}>{basics.name || 'Your Name'}</h1>
          {basics.label && <p style={{ color: primary, fontSize: 14, margin: '4px 0 0' }}>{basics.label}</p>}
          </div>
        </div>
        <div style={{ fontSize: 12, color: '#166534', textAlign: 'right', lineHeight: 1.8 }}>
          {basics.email && <div>{basics.email}</div>}
          {basics.phone && <div>{basics.phone}</div>}
          {basics.location && <div>{basics.location}</div>}
        </div>
        ...
        <div style={{ width: 230, padding: '24px 20px', borderLeft: `2px solid ${primary}20` }}>
          {has(skills) && (
            <div style={{ marginBottom: 24 }}>
              ...
              {skills.map((s, i) => (
                <div key={i} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#14532D' }}>{s.name}</div>
                  {s.level && <div style={{ fontSize: 10, color: '#22C55E' }}>{s.level}</div>}
                  {has(s.keywords) && <div style={{ fontSize: 10, color: '#4ADE80', marginTop: 2 }}>{s.keywords.join(', ')}</div>}
                </div>
              ))}
            </div>
          )}
          {has(education) && (
            <div>
              ...
              {education.map((e, i) => (
                <div key={i} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#14532D' }}>{e.studyType ? `${e.studyType} in ` : ''}{e.area}</div>
                  <div style={{ fontSize: 11, color: '#16A34A' }}>{e.institution}</div>
                  <div style={{ fontSize: 10, color: '#4ADE80' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
                </div>
              ))}
            </div>
          )}
    ...
    <div style={{ ...A4, background: '#fff', display: 'flex' }}>
      ...
      <div style={{ width: 260, padding: '24px 20px' }}>
        ...
        <div style={{ background: primary, borderRadius: 12, padding: '24px 18px', color: dark, marginBottom: 24 }}>
          {basics.avatarUrl && (
            <img src={basics.avatarUrl} alt="" style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', marginBottom: 10, border: '2px solid #422006', display: 'block' }} />
          )}
          <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0, color: '#422006' }}>{basics.name || 'Your Name'}</h1>
          {basics.label && <p style={{ fontSize: 12, margin: '4px 0 0', color: '#713F12' }}>{basics.label}</p>}
          <div style={{ fontSize: 11, marginTop: 12, lineHeight: 1.8, color: '#854D0E' }}>
            {basics.email && <div>{basics.email}</div>}
            {basics.phone && <div>{basics.phone}</div>}
            {basics.location && <div>{basics.location}</div>}
          </div>
        ...
        {has(skills) && (
          <div style={{ marginBottom: 24 }}>
            ...
            {skills.map((s, i) => (
              <div key={i} style={{ marginBottom: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#422006' }}>{s.name}</div>
                {s.level && (
                  <div style={{ background: '#FEF3C7', borderRadius: 4, height: 5, marginTop: 3, overflow: 'hidden' }}>
                    <div style={{ width: `${levelPercent(s.level)}%`, height: '100%', background: primary, borderRadius: 4 }} />
                  </div>
                )}
                {has(s.keywords) && <div style={{ fontSize: 10, color: '#92400E', marginTop: 3 }}>{s.keywords.join(', ')}</div>}
              </div>
            ))}
          </div>
        )}
        {has(education) && (
          <div>
            ...
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#422006' }}>{e.studyType ? `${e.studyType} in ` : ''}{e.area}</div>
                <div style={{ fontSize: 11, color: '#854D0E' }}>{e.institution}</div>
                <div style={{ fontSize: 10, color: '#CA8A04' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Main */}
      <div style={{ flex: 1, padding: '36px 28px', borderLeft: '1px solid #FDE68A' }}>
        {basics.summary && (
          <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            {work.map((w, i) => (
              <div key={i} style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: '#CA8A04' }}>{w.company}</div>
                {w.summary && <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0', lineHeight: 1.6 }}>{w.summary}</p>}
                {has(w.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#374151', lineHeight: 1.7 }}>{w.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
        ...
        {has(projects) && (
          <div>
            ...
            {projects.map((p, i) => (
              <div key={i} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{p.name}</div>
                {p.url && <div style={{ fontSize: 11, color: '#CA8A04' }}>{p.url}</div>}
                {p.description && <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0', lineHeight: 1.6 }}>{p.description}</p>}
                {has(p.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#374151', lineHeight: 1.7 }}>{p.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
...
const Absinthe: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex', fontFamily: "'Roboto', 'Segoe UI', system-ui, sans-serif" }}>
      {/* Sidebar */}
...
const Tanqueray: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
...
const Lagavulin: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: bg, color: text, padding: '36px 40px', fontFamily: "'Consolas', 'Courier New', monospace" }}>
      <div style={{ marginBottom: 28 }}>
        {basics.avatarUrl && (
...
const Macallan: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  const paleBg = '#EBF5FF'
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex', fontFamily: "'Segoe UI', 'Inter', system-ui, sans-serif" }}>
      {/* Sidebar */}
      <div style={{ width: 200, background: paleBg, padding: '32px 16px' }}>
        ...
        <div style={{ fontSize: 11, color: navy, lineHeight: 1.9, marginBottom: 24, borderBottom: `2px solid ${gold}`, paddingBottom: 14 }}>
          {basics.email && <div>{basics.email}</div>}
          {basics.phone && <div>{basics.phone}</div>}
          {basics.location && <div>{basics.location}</div>}
        </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            {skills.map((s, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                ...
                <div style={{ background: '#D6E8FA', borderRadius: 3, height: 6, overflow: 'hidden' }}>
                  <div style={{ width: `${levelPercent(s.level)}%`, height: '100%', background: `linear-gradient(90deg, ${navy}, ${gold})`, borderRadius: 3 }} />
                </div>
          ...
          <div>
            ...
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 10, color: '#4B7399' }}>{e.institution}</div>
                <div style={{ fontSize: 9, color: '#8BABC7' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ flex: 1, padding: '32px 28px' }}>
        <div style={{ marginBottom: 24 }}>
          ...
          {basics.label && <p style={{ color: gold, fontSize: 14, margin: '4px 0 0', fontWeight: 600 }}>{basics.label}</p>}
        </div>
        {basics.summary && (
          <div style={{ marginBottom: 24, padding: '12px 16px', background: paleBg, borderLeft: `3px solid ${gold}`, borderRadius: '0 6px 6px 0' }}>
            <p style={{ fontSize: 13, color: '#334155', lineHeight: 1.7, margin: 0 }}>{basics.summary}</p>
          </div>
        )}
        {has(work) && (
          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: navy, fontWeight: 700, margin: '0 0 14px', borderBottom: `2px solid ${gold}`, paddingBottom: 6, display: 'inline-block' }}>Experience</h3>
            {work.map((w, i) => (
              <div key={i} style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: navy }}>{w.position}</span>
                  <span style={{ fontSize: 11, color: '#8BABC7' }}>{w.startDate}{w.endDate ? ` - ${w.endDate}` : ''}</span>
                </div>
                <div style={{ fontSize: 12, color: gold, fontWeight: 500 }}>{w.company}</div>
                {w.summary && <p style={{ fontSize: 12, color: '#475569', margin: '4px 0 0', lineHeight: 1.6 }}>{w.summary}</p>}
                {has(w.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#475569', lineHeight: 1.7 }}>{w.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
        ...
        {has(projects) && (
          <div>
            <h3 style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: navy, fontWeight: 700, margin: '0 0 14px', borderBottom: `2px solid ${gold}`, paddingBottom: 6, display: 'inline-block' }}>Projects</h3>
            {projects.map((p, i) => (
              <div key={i} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: navy }}>{p.name}</div>
                {p.url && <div style={{ fontSize: 11, color: '#4B7399' }}>{p.url}</div>}
                {p.description && <p style={{ fontSize: 12, color: '#475569', margin: '4px 0 0', lineHeight: 1.6 }}>{p.description}</p>}
                {has(p.highlights) && <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 12, color: '#475569', lineHeight: 1.7 }}>{p.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
              </div>
            ))}
...
const Chartreuse: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  const heading: React.CSSProperties = { fontSize: 13, fontWeight: 700, color: '#064E3B', margin: '0 0 14px', paddingLeft: 14, borderLeft: `4px solid ${green}` }
  return (
    <div style={{ ...A4, background: '#fff', fontFamily: "'Segoe UI', 'Inter', system-ui, sans-serif" }}>
      {/* Header */}
      <Band bg="#fff">
        {basics.avatarUrl && (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${green}` }}>
                <th style={{ textAlign: 'left', padding: '6px 8px', color: '#064E3B', fontWeight: 700 }}>Skill</th>
                <th style={{ textAlign: 'left', padding: '6px 8px', color: '#064E3B', fontWeight: 700 }}>Level</th>
                <th style={{ textAlign: 'left', padding: '6px 8px', color: '#064E3B', fontWeight: 700 }}>Keywords</th>
              </tr>
            </thead>
            <tbody>
              {skills.map((s, i) => (
...
const Glenfiddich: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', fontFamily: "'Segoe UI', 'Inter', system-ui, sans-serif" }}>
      ...
      {/* Career timeline bar */}
      {has(work) && (
        <div style={{ background: '#FFFBEB', padding: '12px 36px', borderBottom: `2px solid ${amber}`, overflowX: 'auto' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            {work.map((w, i) => (
              <div key={i} style={{ marginBottom: 16, display: 'flex', gap: 16 }}>
                {/* Metric callout */}
                <div style={{ width: 70, flexShrink: 0, textAlign: 'center', padding: '8px 0', background: '#FFFBEB', borderRadius: 8, border: `1px solid ${amber}40` }}>
          ...
          <div style={{ flex: 1 }}>
            {has(skills) && (
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {skills.map((s, i) => (
                    <span key={i} style={{ fontSize: 11, background: '#FFFBEB', color: '#92400E', padding: '4px 10px', borderRadius: 4, border: `1px solid ${amber}40`, fontWeight: 500 }}>
                      {s.name}{s.level ? ` (${s.level})` : ''}
                    </span>
                  ))}
                </div>
...
const Courvoisier: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', display: 'flex', fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
      {/* Indigo strip */}
      ...
      <div style={{ width: 230, background: sidebarBg, padding: '32px 18px' }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 700, color: '#312E81', margin: 0 }}>{basics.name || 'Your Name'}</h1>
        {basics.label && <p style={{ color: indigo, fontSize: 12, margin: '4px 0 0' }}>{basics.label}</p>}
        <div style={{ marginTop: 16, fontSize: 11, color: '#6366F1', lineHeight: 1.9 }}>
          {basics.email && <div>{basics.email}</div>}
          {basics.phone && <div>{basics.phone}</div>}
          {basics.location && <div>{basics.location}</div>}
        </div>
        {has(skills) && (
          <div style={{ marginTop: 24 }}>
            <h3 style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.5, color: '#312E81', fontWeight: 700, margin: '0 0 12px' }}>Skills</h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'baseline' }}>
              {skills.map((s, i) => (
                <span key={i} style={{ fontSize: fontSizeForLevel(s.level), fontWeight: fontSizeForLevel(s.level) > 13 ? 700 : 500, color: '#4338CA', lineHeight: 1.4 }}>
                  {s.name}
                </span>
              ))}
            </div>
        ...
        {has(education) && (
          <div style={{ marginTop: 24 }}>
            <h3 style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.5, color: '#312E81', fontWeight: 700, margin: '0 0 10px' }}>Education</h3>
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#312E81' }}>{e.studyType ? `${e.studyType} in ` : ''}{e.area}</div>
                <div style={{ fontSize: 10, color: '#6366F1' }}>{e.institution}</div>
                <div style={{ fontSize: 9, color: '#A5B4FC' }}>{e.startDate}{e.endDate ? ` - ${e.endDate}` : ''}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ flex: 1, padding: '32px 28px' }}>
        {basics.summary && (
          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: '#312E81', fontWeight: 700, margin: '0 0 10px' }}>Summary</h3>
            <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, margin: 0 }}>{basics.summary}</p>
          </div>
        )}
        {has(work) && (
          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: '#312E81', fontWeight: 700, margin: '0 0 14px' }}>Experience</h3>
            {work.map((w, i) => (
        ...
        {has(projects) && (
          <div>
            <h3 style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: 1.5, color: '#312E81', fontWeight: 700, margin: '0 0 14px' }}>Projects</h3>
            {projects.map((p, i) => (
...
const AmaroneTpl: React.FC<{ data: ResumeData; color: string }> = ({ data, color }) => {
  ...
  return (
    <div style={{ ...A4, background: '#fff', padding: '36px 40px' }}>
      ...
      {/* Experience - even (right border, right aligned) */}
      {has(work) && (
        <div style={{ marginBottom: 28, borderRight: `4px solid ${primary}`, paddingRight: 20, textAlign: 'right' }}>
          <h3 style={{ ...heading, textAlign: 'right' }}>Experience</h3>
          {work.map((w, i) => (
            <div key={i} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{w.position}</div>
              <div style={{ fontSize: 12, color: primary }}>{w.company} | {w.startDate}{w.endDate ? ` - ${w.endDate}` : ''}</div>
              {w.summary && <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0', lineHeight: 1.6 }}>{w.summary}</p>}
              {has(w.highlights) && <ul style={{ margin: '4px 0 0', paddingRight: 16, fontSize: 12, color: '#374151', lineHeight: 1.7, listStylePosition: 'inside' }}>{w.highlights.map((h, j) => h && <li key={j}>{h}</li>)}</ul>}
            </div>
          ))}
        </div>
      )}
      ...
      {/* Skills - even (right border, right aligned) */}
      {has(skills) && (
        <div style={{ marginBottom: 28, borderRight: `4px solid ${primary}`, paddingRight: 20, textAlign: 'right' }}>
          <h3 style={{ ...heading, textAlign: 'right' }}>Skills</h3>
